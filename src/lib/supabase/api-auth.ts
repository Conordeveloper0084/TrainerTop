 ...
 *
 * BAN qilingan foydalanuvchi "kirmagan" deb hisoblanadi (user: null) va `banned` maydonida
 * sababi qaytadi — shu tufayli barcha API route'lar avtomatik ravishda ban'ni hurmat qiladi.
 */
export async function getApiUser(
  request: NextRequest
): Promise<{ supabase: ReturnType<typeof createApiSupabaseClient>; user: User | null; banned: ActiveBan | null }> {
  const supabase = createApiSupabaseClient(request);
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (user) {
    const banned = await getActiveBan(user.id);
    if (banned) return { supabase, user: null, banned };
  }
  return { supabase, user, banned: null };
}
