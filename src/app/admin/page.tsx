...
import { Users, UserCheck, BookOpen, MessageCircle, Wallet, TrendingUp, Loader2, Clock, ArrowRight, ArrowUpRight, ShoppingCart, Percent } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [s, t, p, sales] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      ...
      {stats.unreconciled > 0 && (
        <div className="card p-4 border-red-500/30 bg-red-500/[0.04]">
          <p className="text-sm font-semibold text-red-400">Diqqat: {stats.unreconciled} ta trenerning balansi hisob daftariga mos kelmayapti</p>
          <p className="text-xs text-white/50 mt-1">Bu holat bo'lmasligi kerak. Pul yechishni tasdiqlashdan oldin tekshiring (SQL: trainer_ledger jadvali).</p>
        </div>
      )}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <FinanceCard icon={ShoppingCart} label="Jami sotuvlar" value={stats.total_sales?.toString() || "0"} sub="Muvaffaqiyatli to'lovlar" />
          <FinanceCard icon={TrendingUp} label="Jami tushum" value={formatPrice(stats.total_revenue || 0)} sub="Barcha to'lovlar" />
          <FinanceCard icon={Percent} label="Platforma daromadi" value={formatPrice(stats.total_commission || 0)} sub={`Oxirgi 30 kun: ${formatPrice(stats.commission_30d || 0)}`} />
          <FinanceCard icon={Wallet} label="Trenerlar balansi" value={formatPrice(stats.total_balance)} sub="Yechishga tayyor" />
          <FinanceCard icon={Clock} label="Pul yechish" value={stats.pending_payouts.toString()} sub={stats.pending_payouts > 0 ? `Kutilayotgan: ${formatPrice(stats.pending_payout_amount || 0)}` : "Kutilayotgan yo'q"} alert={stats.pending_payouts > 0} />
        </div>
      </div>
      ...
      <div className="card">
        <div className="flex items-center justify-between p-5 border-b border-white/[0.04]">
          <div>
            <h2 className="font-semibold text-sm">Oxirgi sotuvlar</h2>
            <p className="text-[11px] text-white/40 mt-0.5">Barcha to'lovlar</p>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-xs text-lime font-semibold">{recentSales.length} ta</span>
          </div>
        </div>
        {recentSales.length === 0 ? (
          <div className="p-8 text-center"><p className="text-white/30 text-sm">Hali sotuv yo'q</p></div>
        ) : (
          <div className="divide-y divide-white/[0.04]">
            {recentSales.map((sale: any) => (
              <div key={sale.id} className="flex items-center gap-3 p-4">
                <div className="w-9 h-9 rounded-full bg-lime/10 flex items-center justify-center shrink-0">
                  <TrendingUp className="h-4 w-4 text-lime" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">{sale.lesson?.title || "Darslik"}</p>
                  <p className="text-[10px] text-white/30">
                    {sale.buyer?.full_name || "User"} → {sale.trainer?.full_name || "Trener"}
                  </p>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-sm font-bold text-lime">{formatPrice(sale.amount)}</p>
                  <p className="text-[9px] text-white/20">
                    {sale.created_at ? new Date(sale.created_at).toLocaleString("uz-UZ", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" }) : ""}
                  </p>
                </div>
              </div>
            ))}
