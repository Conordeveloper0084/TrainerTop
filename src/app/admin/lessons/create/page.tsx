...
interface VideoItem { id: string; title: string; description: string; url: string; uploading: boolean; videoType: "upload" | "youtube"; }
interface Section { id: string; title: string; content: string; videos: VideoItem[]; }

// YouTube video ID ni havoladan ajratib olish
...
function CreateLessonPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/admin/lessons/${editId}`);
        ...
        if (loadedSections.length > 0) {
          setSections(loadedSections.map((s: any, i: number) => ({
            ...
            videos: (s.videos || []).map((v: any, vi: number) => ({
              ...
              url: v.url || "",
              uploading: false,
              videoType: (v.videoType === "youtube" || (v.url && (v.url.includes("youtube.com") || v.url.includes("youtu.be")))) ? "youtube" : "upload",
            })),
          })));
        }
  ...
  const handleCover = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!f.type.startsWith("image/")) { toast.error("Faqat rasm fayl"); return; }
    setCoverFile(f);
    setCoverPreview(URL.createObjectURL(f));
  };

  // Cover va rasm yuklash — universal funksiya
  const uploadFile = async (file: File, folder: string): Promise<string> => {
    // Kichik fayllar (< 4MB) → server orqali
    ...
    // Katta fayllar → presigned URL
    const presignRes = await fetch("/api/upload", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ fileName: file.name, fileType: file.type, fileSize: file.size, folder }),
    });
    if (!presignRes.ok) throw new Error("Presigned URL xatolik");
    const { presignedUrl, publicUrl } = await presignRes.json();

    const uploadRes = await fetch(presignedUrl, { method: "PUT", body: file, headers: { "Content-Type": file.type } });
    if (!uploadRes.ok) throw new Error("Yuklash xatolik");
    return publicUrl;
  };

  // Video qo'shish — avval nom/tavsif, keyin upload (huddi modul kabi)
  const addVideo = (sectionId: string) => {
    const videoId = `v-${Date.now()}`;
    const newVideo: VideoItem = { id: videoId, title: "", description: "", url: "", uploading: false, videoType: "upload" };
    setSections((prev) => prev.map((s) => s.id === sectionId ? { ...s, videos: [...s.videos, newVideo] } : s));
  };

  // Video yuklash — chunked upload (server orqali, CORS muammo yo'q)
  const handleVideoUpload = async (sectionId: string, videoId: string, e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (!file.type.startsWith("video/")) { toast.error("Faqat video fayl (MP4, MOV)"); return; }
    if (file.size > 500 * 1024 * 1024) { toast.error("Video 500MB dan katta"); return; }
    e.target.value = "";

    const sizeMB = (file.size / 1024 / 1024).toFixed(1);

    // Uploading holatini ko'rsatish
    setSections((prev) => prev.map((s) => s.id === sectionId ? { ...s, videos: s.videos.map((v) => v.id === videoId ? { ...v, uploading: true } : v) } : s));

    try {
      const CHUNK_SIZE = 3.5 * 1024 * 1024; // 3.5MB — Vercel limitdan kichik
      const totalChunks = Math.ceil(file.size / CHUNK_SIZE);

      if (file.size <= CHUNK_SIZE) {
        // KICHIK FAYL — oddiy upload
        const formData = new FormData();
        formData.append("file", file);
        formData.append("bucket", "lessons");
        const res = await fetch("/api/upload", { method: "POST", body: formData });
        if (!res.ok) { const d = await res.json().catch(() => ({})); throw new Error(d.message || "Yuklash xatolik"); }
        const data = await res.json();
        const publicUrl = data.url || data.publicUrl;

        setSections((prev) => prev.map((s) => s.id === sectionId ? {
          ...s, videos: s.videos.map((v) => v.id === videoId ? { ...v, url: publicUrl, uploading: false } : v)
        } : s));
      } else {
        // KATTA FAYL — chunked multipart upload
        toast.info(`Video yuklanmoqda (${sizeMB} MB, ${totalChunks} bo'lak)...`);

        // 1. Multipart upload boshlash
        const initRes = await fetch("/api/upload/init", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ fileName: file.name, fileType: file.type, folder: "lessons" }),
        });
        if (!initRes.ok) throw new Error("Upload boshlanmadi");
        const { uploadId, key, publicUrl } = await initRes.json();

        // 2. Har bir chunk'ni yuklash
        ...
        for (let i = 0; i < totalChunks; i++) {
          ...
          const formData = new FormData();
          formData.append("chunk", new Blob([chunk]), `chunk-${partNumber}`);
          formData.append("uploadId", uploadId);
          formData.append("key", key);
          formData.append("partNumber", partNumber.toString());

          const partRes = await fetch("/api/upload/part", { method: "POST", body: formData });
          if (!partRes.ok) throw new Error(`Bo'lak ${partNumber}/${totalChunks} yuklanmadi`);
          const partData = await partRes.json();
          parts.push({ partNumber: partData.partNumber, etag: partData.etag });
        }

        // 3. Multipart upload tugatish
        const completeRes = await fetch("/api/upload/complete", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ uploadId, key, parts }),
        });
        if (!completeRes.ok) throw new Error("Upload tugatilmadi");
      ...
      // Agar sarlavha bo'sh — fayl nomidan
      setSections((prev) => prev.map((s) => s.id === sectionId ? {
        ...s, videos: s.videos.map((v) => v.id === videoId && !v.title ? { ...v, title: file.name.replace(/\.[^.]+$/, "") } : v)
      } : s));

      toast.success(`Video yuklandi! (${sizeMB} MB)`);
    } catch (err: any) {
      console.error("Video upload error:", err);
      setSections((prev) => prev.map((s) => s.id === sectionId ? {
        ...s, videos: s.videos.map((v) => v.id === videoId ? { ...v, uploading: false } : v)
      } : s));
      toast.error(err.message || "Video yuklashda xatolik");
    }
  };
  ...
  // Video turini almashtirish (yuklash / YouTube)
  const setVideoType = (sectionId: string, videoId: string, type: "upload" | "youtube") => {
    setSections((prev) => prev.map((s) => s.id === sectionId ? { ...s, videos: s.videos.map((v) => v.id === videoId ? { ...v, videoType: type, url: "" } : v) } : s));
  };

  // YouTube linkni saqlash (validatsiya bilan)
  const updateYoutubeUrl = (sectionId: string, videoId: string, rawUrl: string) => {
    setSections((prev) => prev.map((s) => s.id === sectionId ? { ...s, videos: s.videos.map((v) => v.id === videoId ? { ...v, url: rawUrl.trim() } : v) } : s));
  };

  // Video tartibini o'zgartirish
  const moveVideo = (sectionId: string, i: number, d: number) => {
    setSections((prev) => prev.map((s) => {
      if (s.id !== sectionId) return s;
      const n = i + d;
      if (n < 0 || n >= s.videos.length) return s;
      const a = [...s.videos]; [a[i], a[n]] = [a[n], a[i]];
      return { ...s, videos: a };
    }));
  };
  ...
  const handleSave = async (publish: boolean) => {
    ...
    try {
      ...
      if (coverFile) {
        try {
          coverUrl = await uploadFile(coverFile, "lessons");
        } catch { toast.error("Muqova rasmi yuklanmadi"); }
      }
      ...
      const body = {
        ...
        difficulty, cover_url: coverUrl,
        sections: sections.map((s) => ({
          title: s.title, content: s.content,
          videos: s.videos.filter((v) => v.url).map((v) => ({ title: v.title, description: v.description || "", url: v.url, videoType: v.videoType || "upload" })),
        })),
        status: publish ? "published" : "draft",
      };

      const url = isEdit ? `/api/admin/lessons/${editId}` : "/api/admin/lessons";
  ...
  return (
    <div className="container-main py-6 max-w-3xl mx-auto">
      <Link href="/admin/lessons" className="inline-flex items-center gap-1.5 text-sm text-white/40 hover:text-white mb-6"><ChevronLeft className="h-4 w-4" />Orqaga</Link>
      <h1 className="text-xl font-bold mb-6">{isEdit ? "Platforma darsligini tahrirlash" : "Yangi platforma darsligi"}</h1>
            ...
            <div key={section.id} className="border border-white/[0.06] rounded-xl overflow-hidden">
              ...
              <div className="p-4 space-y-4">
                ...
                <div>
                  ...
                  {section.videos.length > 0 ? (
                    <div className="space-y-2 mb-3">
                      {section.videos.map((video, vi) => (
                        <div key={video.id} className={cn("rounded-xl border transition-all",
                          video.url ? "bg-dark-card border-lime/10" : video.uploading ? "bg-dark-card border-yellow-500/20" : "bg-dark-card/80 border-white/[0.06]")}>
                          {/* Header — raqam, status, actions */}
                          <div className="flex items-center justify-between px-4 py-2.5 border-b border-white/[0.04]">
                            <div className="flex items-center gap-2">
                              <div className={cn("w-6 h-6 rounded-md flex items-center justify-center text-[10px] font-bold",
                                video.url ? "bg-lime/10 text-lime" : "bg-white/[0.06] text-white/30")}>
                                {vi + 1}
                              </div>
                              <span className="text-[10px] text-white/30">Video {vi + 1}</span>
                              {video.url && <span className="text-[8px] bg-lime/10 text-lime px-1.5 py-0.5 rounded-full">Tayyor</span>}
                              {video.uploading && <span className="text-[8px] bg-yellow-500/10 text-yellow-500 px-1.5 py-0.5 rounded-full flex items-center gap-1"><Loader2 className="h-2.5 w-2.5 animate-spin" />Yuklanmoqda</span>}
                            </div>
                            <div className="flex items-center gap-1">
                              <button onClick={() => moveVideo(section.id, vi, -1)} disabled={vi === 0} className="p-1 text-white/15 hover:text-white/40 disabled:opacity-20 rounded hover:bg-white/[0.04]"><ChevronUp className="h-3.5 w-3.5" /></button>
                              <button onClick={() => moveVideo(section.id, vi, 1)} disabled={vi === section.videos.length - 1} className="p-1 text-white/15 hover:text-white/40 disabled:opacity-20 rounded hover:bg-white/[0.04]"><ChevronDown className="h-3.5 w-3.5" /></button>
                              <button onClick={() => removeVideo(section.id, video.id)} className="p-1 text-red-400/20 hover:text-red-400 rounded hover:bg-red-500/[0.06]"><Trash2 className="h-3.5 w-3.5" /></button>
                            </div>
                          </div>

                          {/* Body — nom, tavsif, yuklash */}
                          <div className="p-4 space-y-3">
                            <div>
                              <label className="block text-[10px] text-white/30 mb-1.5 uppercase tracking-wider">Sarlavha *</label>
                              <input type="text" value={video.title} onChange={(e) => updateVideoTitle(section.id, video.id, e.target.value)}
                                className="input-field text-sm" placeholder="Masalan: Bench Press texnikasi" />
                            </div>
                            <div>
                              <label className="block text-[10px] text-white/30 mb-1.5 uppercase tracking-wider">Tavsif (ixtiyoriy)</label>
                              <input type="text" value={video.description || ""} onChange={(e) => updateVideoDesc(section.id, video.id, e.target.value)}
                                className="input-field text-xs" placeholder="Masalan: To'g'ri texnika va keng xatolar" />
                            </div>

                            {/* Video turi tanlash — Yuklash yoki YouTube (faqat admin) */}
                            <div className="flex gap-2">
                              <button type="button" onClick={() => setVideoType(section.id, video.id, "upload")}
                                className={cn("flex-1 text-[11px] py-2 rounded-lg border transition-all",
                                  video.videoType !== "youtube" ? "bg-lime/10 border-lime/30 text-lime font-medium" : "bg-white/[0.02] border-white/[0.08] text-white/40 hover:text-white/60")}>
                                Video yuklash
                              </button>
                              <button type="button" onClick={() => setVideoType(section.id, video.id, "youtube")}
                                className={cn("flex-1 text-[11px] py-2 rounded-lg border transition-all",
                                  video.videoType === "youtube" ? "bg-red-500/10 border-red-500/30 text-red-400 font-medium" : "bg-white/[0.02] border-white/[0.08] text-white/40 hover:text-white/60")}>
                                YouTube link
                              </button>
                            </div>

                            {/* YouTube link kiritish */}
                            {video.videoType === "youtube" ? (
                              <div className="space-y-2">
                                <input type="text" value={video.url} onChange={(e) => updateYoutubeUrl(section.id, video.id, e.target.value)}
                                  className="input-field text-xs" placeholder="https://www.youtube.com/watch?v=..." />
                                {video.url && getYoutubeId(video.url) ? (
                                  <div className="flex items-center gap-3 p-3 bg-red-500/[0.04] border border-red-500/10 rounded-lg">
                                    <div className="w-9 h-9 rounded-lg bg-red-500/10 flex items-center justify-center shrink-0">
                                      <Play className="h-4 w-4 text-red-400" />
                                    </div>
                                    <div className="flex-1">
                                      <p className="text-xs text-red-400 font-medium">YouTube video tayyor</p>
                                      <p className="text-[10px] text-white/30 truncate">{video.url}</p>
                                    </div>
                                  </div>
                                ) : video.url ? (
                                  <p className="text-[10px] text-yellow-500">Noto'g'ri YouTube havola. To'g'ri formatda kiriting.</p>
                                ) : (
                                  <p className="text-[10px] text-white/20">YouTube videoning to'liq havolasini joylashtiring</p>
                                )}
                              </div>
                            ) : !video.url && !video.uploading ? (
                              <label className="flex items-center gap-3 p-3 bg-white/[0.02] hover:bg-lime/[0.04] border border-dashed border-white/[0.08] hover:border-lime/30 rounded-lg cursor-pointer transition-all group">
                                <div className="w-9 h-9 rounded-lg bg-white/[0.04] group-hover:bg-lime/10 flex items-center justify-center">
                                  <Upload className="h-4 w-4 text-white/20 group-hover:text-lime transition-colors" />
                                </div>
                                <div>
                                  <p className="text-xs text-white/50 group-hover:text-white/70 font-medium">Video faylni tanlash</p>
                                  <p className="text-[10px] text-white/20">MP4, MOV · 500MB gacha</p>
                                </div>
                                <input type="file" accept="video/*" onChange={(e) => handleVideoUpload(section.id, video.id, e)} className="hidden" />
                              </label>
                            ) : video.uploading ? (
                              <div className="flex items-center gap-3 p-3 bg-yellow-500/[0.04] border border-yellow-500/10 rounded-lg">
                                <Loader2 className="h-4 w-4 text-yellow-500 animate-spin shrink-0" />
                                <div>
                                  <p className="text-xs text-yellow-500 font-medium">Yuklanmoqda...</p>
                                  <p className="text-[10px] text-yellow-500/60">Sahifani yopmang</p>
                                </div>
                              </div>
                            ) : (
                              <div className="flex items-center gap-3 p-3 bg-lime/[0.04] border border-lime/10 rounded-lg">
                                <div className="w-9 h-9 rounded-lg bg-lime/10 flex items-center justify-center shrink-0">
                                  <Play className="h-4 w-4 text-lime" />
                                </div>
                                <div className="flex-1">
                                  <p className="text-xs text-lime font-medium">Video yuklangan</p>
                                  <p className="text-[10px] text-white/30 truncate">{video.url?.split("/").pop()}</p>
                                </div>
                                <label className="text-[10px] text-white/40 hover:text-lime cursor-pointer px-2 py-1 rounded hover:bg-lime/10 transition-all">
                                  Almashtirish
                                  <input type="file" accept="video/*" onChange={(e) => handleVideoUpload(section.id, video.id, e)} className="hidden" />
                                </label>
                              </div>
                  ...
                  {/* Video qo'shish tugmasi — modul kabi */}
                  <button onClick={() => addVideo(section.id)}
                    className="w-full flex items-center justify-center gap-2 py-2.5 border border-dashed border-white/10 rounded-xl hover:border-lime/30 hover:bg-lime/[0.02] transition-all text-xs text-white/40 hover:text-lime group">
                    <Plus className="h-3.5 w-3.5" />Video qo'shish
                  </button>
                </div>
              </div>
            </div>
