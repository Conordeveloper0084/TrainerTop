...
import { LayoutDashboard, Users, Wallet, ArrowLeft, Loader2, Shield, UserCheck, BookOpen, Sun, Moon, Settings, LifeBuoy, MessageSquare, Megaphone, Star, Bell } from "lucide-react";
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const links = [
    ...
    { href: "/admin/lessons", icon: BookOpen, label: "Darsliklar" },
    { href: "/admin/payouts", icon: Wallet, label: "Pul yechish" },
    { href: "/admin/groups", icon: MessageSquare, label: "Guruhlar" },
    { href: "/admin/channel", icon: Megaphone, label: "Rasmiy kanal" },
    { href: "/admin/announcements", icon: Bell, label: "E'lonlar" },
    { href: "/admin/reviews", icon: Star, label: "Baholar" },
    { href: "/admin/support", icon: LifeBuoy, label: "Murojaatlar" },
    ...
    <div className="min-h-screen bg-dark">
      ...
      <div className="container-main flex gap-6 py-6">
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-dark-surface border-t border-white/[0.06] pb-safe">
          <nav className="grid grid-cols-5 gap-0.5 p-2">
            {links.map((l) => <AdminLinkMobile key={l.href} {...l} pathname={pathname} />)}
          </nav>
        </div>

        <main className="flex-1 min-w-0 pb-36 md:pb-0">{children}</main>
      </div>
    </div>
  );
}
...
function AdminLinkMobile({ href, icon: Icon, label, exact, pathname }: any) {
  const isActive = exact ? pathname === href : pathname.startsWith(href);
  const shortLabel = label === "Foydalanuvchilar" ? "Userlar" : label === "Pul yechish" ? "To'lovlar" : label === "Darsliklar" ? "Darslik" : label;
  return (
