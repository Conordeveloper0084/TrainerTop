...
export async function POST(request: NextRequest) {
  try {
    ...
    if (lesson && transaction.user_id) {
      ...
      // 3. Purchase type aniqlash (oylik yoki umrbod)
      const isMonthly = transaction.merchant_trans_id?.includes("_monthly") || 
        (lesson.pricing_model === "monthly") ||
        (lesson.pricing_model === "both" && purchaseAmount === (lesson.price_monthly || 0));
      
      const purchaseType = isMonthly ? "monthly" : "lifetime";
