...
export async function GET() {
  try {
    ...
    const { data: salesData, count: salesCount } = await supabaseAdmin
      .from("purchases").select("amount, commission", { count: "exact" }).eq("status", "paid");
    const totalRevenue = (salesData || []).reduce((s: number, p: any) => s + (p.amount || 0), 0);
    const totalCommission = (salesData || []).reduce((s: number, p: any) => s + (p.commission || 0), 0);
