...
// GET /api/chat/[id]/messages (cookie yoki Bearer token)
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: conv } = await supabaseAdmin
      .from("conversations").select("trainer_id, user_id, trainer_unread, user_unread").eq("id", params.id).single();

    if (conv) {
      const unreadField = conv.trainer_id === user.id ? "user_unread" : "trainer_unread";
      const currentUnread = (conv.trainer_id === user.id ? conv.user_unread : conv.trainer_unread) || 0;
