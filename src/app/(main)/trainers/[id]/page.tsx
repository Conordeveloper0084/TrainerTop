...
export default function TrainerProfilePage() {
  ...
  return (
    <div className="container-main py-6">
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {tp.is_verified && <AthleteBadge size={22} />}
                  {isOwnProfile && <span className="text-[10px] bg-lime-muted text-lime px-2 py-0.5 rounded-full">Siz</span>}
                </div>
                {tp.profiles?.username && <p className="text-sm text-lime/70 -mt-0.5 mb-2">@{tp.profiles.username}</p>}
                {tp.is_verified && <div className="mb-2"><AthleteBadge label size={16} /></div>}
      ...
      {tab === "posts" && (tp.posts?.length > 0 ? <div className="max-w-xl mx-auto space-y-4">{tp.posts.map((p: any) => <div key={p.id} className="card overflow-hidden"><PostMedia post={p} /><div className="p-4">{p.caption && <p className="text-sm text-white/60">{p.caption}</p>}<div className="flex items-center gap-4 mt-3 text-xs text-white/30"><span className="flex items-center gap-1"><Heart className="h-3.5 w-3.5" />{p.likes_count || 0}</span><span className="flex items-center gap-1"><MessageCircle className="h-3.5 w-3.5" />{p.comments_count || 0}</span></div></div></div>)}</div> : <div className="card p-8 text-center"><p className="text-sm text-white/40">Hali post yo'q</p></div>)}
