  ...
  return (
    <div className="container-main py-8">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1"><Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-white/30" /><input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Ism yoki @username qidirish..." className="input-field !pl-11" /></div>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="min-w-0">
            <h3 className="font-bold text-base group-hover:text-lime transition-colors flex items-center gap-1.5 min-w-0"><span className="truncate">{name}</span>{t.is_verified && <AthleteBadge size={16} />}</h3>
            {t.profiles?.username && <p className="text-[11px] text-white/30">@{t.profiles.username}</p>}
          </div>
          {t.rating > 0 && (
