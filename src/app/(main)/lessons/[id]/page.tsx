                      ...
                      <div className="space-y-1.5 pl-10">
                        {s.videos.map((v: any, vi: number) => (
                          <div key={vi} className={cn("flex items-center gap-3 p-3 rounded-lg transition-colors group",
                            activeVideo?.url === v.url ? "bg-lime/10 border border-lime/20" : "bg-dark-card/50 hover:bg-dark-card",
                            ...
                            <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center shrink-0",
                              activeVideo?.url === v.url ? "bg-lime/20" : hasAccess ? "bg-lime/10 group-hover:bg-lime/20" : "bg-white/[0.04]")}>
                            ...
                            <div className="flex-1 min-w-0">
                              <p className={cn("text-xs font-medium truncate", activeVideo?.url === v.url ? "text-lime" : "")}>{v.title || `Video ${vi + 1}`}</p>
                            ...
                            {hasAccess && v.url ? (
                              <span className={cn("text-[10px] font-semibold shrink-0",
                                activeVideo?.url === v.url ? "text-lime" : "text-lime opacity-0 group-hover:opacity-100 transition-opacity")}>
                                {activeVideo?.url === v.url ? "Ko'rilmoqda" : "Ko'rish →"}
                              </span>
                            ) : !hasAccess ? (
...
// ====== SOTIB OLISH TUGMASI ======
function BuyButton({ lessonId, priceType }: { lessonId: string; priceType: string }) {
  ...
  return (
    <button onClick={handleBuy} disabled={loading}
      className="btn-lime w-full !py-3 text-sm flex items-center justify-center gap-2 disabled:opacity-50">
