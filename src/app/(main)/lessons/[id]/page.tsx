...
import { ChevronLeft, Star, Users, BookOpen, ShoppingCart, Play, Lock, Clock, Loader2, MessageCircle, Send, Dumbbell, Volume2, CheckCircle } from "lucide-react";
...
// YouTube video ID ni havoladan ajratish
...
export default function LessonDetailWrapper() {
  return (
...
function LessonDetailPage() {
  ...
  const [activeVideo, setActiveVideo] = useState<{ title: string; url: string; description?: string } | null>(null);
  ...
  useEffect(() => {
    if (searchParams.get("payment") === "success") {
      toast.success("To'lov muvaffaqiyatli! Darslikka kirish ochildi.");
      window.history.replaceState({}, "", `/lessons/${params.id}`);
    }
  }, [searchParams]);
  ...
  // Barcha videolar — flat list
  const allVideos = sections.flatMap((s: any) => (s.videos || []).map((v: any) => ({ ...v, sectionTitle: s.title })));
    ...
    <div className="container-main py-6 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Video Player yoki Cover */}
          <div className="w-full aspect-video rounded-2xl bg-dark-card overflow-hidden flex items-center justify-center relative">
            {activeVideo ? (
              <>
                ...
                <div className="absolute top-0 left-0 right-0 bg-gradient-to-b from-black/70 to-transparent p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      {activeVideo.description && <p className="text-[11px] text-white/60 mt-0.5">{activeVideo.description}</p>}
                    </div>
                    <button onClick={() => setActiveVideo(null)} className="text-white/60 hover:text-white text-xs bg-black/30 px-3 py-1.5 rounded-lg">
                      ✕ Yopish
                    </button>
                  </div>
                </div>
              ...
              <img src={lesson.cover_image_url} alt="" className="w-full h-full object-cover" />
            ) : (
          ...
          {/* Modullar */}
          {sections.length > 0 && (
            <div className="card p-6" data-section="modules">
                    ...
                    {s.videos?.length > 0 && (
                      <div className="space-y-1.5 pl-10">
                        {s.videos.map((v: any, vi: number) => (
                          <div key={vi} className={cn("flex items-center gap-3 p-3 rounded-lg transition-colors group",
                            (!!v.url && activeVideo?.url === v.url) ? "bg-lime/10 border border-lime/20" : "bg-dark-card/50 hover:bg-dark-card",
                            hasAccess && v.url ? "cursor-pointer" : "")}
                            onClick={() => hasAccess && v.url && setActiveVideo({ title: v.title || `Video ${vi + 1}`, url: v.url, description: v.description })}>
                            <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center shrink-0",
                              (!!v.url && activeVideo?.url === v.url) ? "bg-lime/20" : hasAccess ? "bg-lime/10 group-hover:bg-lime/20" : "bg-white/[0.04]")}>
                              {(!!v.url && activeVideo?.url === v.url) ? (
                            ...
                            <div className="flex-1 min-w-0">
                              <p className={cn("text-xs font-medium truncate", (!!v.url && activeVideo?.url === v.url) ? "text-lime" : "")}>{v.title || `Video ${vi + 1}`}</p>
                            ...
                            {hasAccess && v.url ? (
                              <span className={cn("text-[10px] font-semibold shrink-0",
                                (!!v.url && activeVideo?.url === v.url) ? "text-lime" : "text-lime opacity-0 group-hover:opacity-100 transition-opacity")}>
                                {(!!v.url && activeVideo?.url === v.url) ? "Ko'rilmoqda" : "Ko'rish →"}
                              </span>
                            ) : !hasAccess ? (
                              <span className="text-[10px] text-white/20 shrink-0">Sotib oling</span>
                            ) : null}
                          </div>
                        ))}
                      </div>
        ...
        <div className="space-y-4">
          <div className="card p-6 sticky top-20">
            <PricingDisplay lesson={lesson} isOwner={isOwner} isPurchased={lesson.is_purchased} />
...
function PricingDisplay({ lesson, isOwner, isPurchased }: any) {
  ...
  // SOTIB OLINGAN — boshqa ko'rinish
  if (isPurchased && !isOwner) {
    const isMonthly = lesson.purchase_type === "monthly";
    const daysLeft = lesson.days_left;

    return (
      <div className="text-center">
        <div className="w-14 h-14 rounded-full bg-lime/10 flex items-center justify-center mx-auto mb-3">
          <CheckCircle className="h-7 w-7 text-lime" />
        </div>
        <p className="text-sm font-semibold mb-1">Darslik sizniki!</p>
        
        {isMonthly ? (
          <div className="mb-4">
            <div className={cn("inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium",
              daysLeft <= 5 ? "bg-red-500/10 text-red-400" : daysLeft <= 15 ? "bg-yellow-500/10 text-yellow-500" : "bg-lime/10 text-lime")}>
              <Clock className="h-3 w-3" />
              {daysLeft} kun qoldi
            </div>
            <p className="text-[10px] text-white/30 mt-2">Oylik obuna · {new Date(lesson.expires_at).toLocaleDateString("uz")} gacha</p>
          </div>
        ) : (
          <div className="mb-4">
            <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-lime/10 text-lime">
              ∞ Umrbod kirish
            </div>
          </div>
        )}

        <button onClick={() => { const el = document.querySelector('[data-section="modules"]'); el?.scrollIntoView({ behavior: 'smooth' }); }}
          className="btn-lime w-full !py-3 text-sm flex items-center justify-center gap-2">
          <Play className="h-4 w-4" />Videolarni ko'rish
        </button>
        <Link href={`/chat?lesson=${lesson.id}`} className="btn-outline w-full !py-3 text-sm flex items-center justify-center gap-2 mt-2">
          <MessageCircle className="h-4 w-4" />Darslik guruhi
        </Link>

        {isMonthly && daysLeft <= 5 && (
          <p className="text-[10px] text-red-400/70 mt-3">Obuna tugayapti! Umrbod variantni ko'rib chiqing.</p>
        )}
      </div>
    );
  }
  ...
  if (!isPurchased && lesson.purchase_type === "monthly" && lesson.days_left !== null && lesson.days_left <= 0 && !isOwner) {
    return (
      <div className="text-center">
        ...
        <p className="text-sm font-semibold mb-1 text-red-400">Obuna muddati tugadi</p>
        <p className="text-[11px] text-white/40 mb-4">Davom etish uchun qayta sotib oling</p>
        <BuyButton lessonId={lesson.id} priceType="monthly" />
        <p className="text-[10px] text-white/30 mt-3">yoki umrbod variant tanlang</p>
        <button onClick={() => {}} className="text-xs text-lime mt-1 hover:underline">Umrbod sotib olish — {formatPrice(priceLifetime)}</button>
        <p className="text-[10px] text-white/30 mt-3">Darslik guruhi ham obuna yangilanguncha yopiq turadi</p>
      </div>
    );
  }
    ...
    return (
      <>
        ...
        <div className="space-y-2 mb-4">
          ...
          <button onClick={() => setSelected("lifetime")}
        ...
        <BuyButton lessonId={lesson.id} priceType={selected} />
      </>
    );
  }
  ...
  return (
    <>
      ...
      <BuyButton lessonId={lesson.id} priceType={isMonthlyOnly ? "monthly" : "lifetime"} />
    </>
  );
}
