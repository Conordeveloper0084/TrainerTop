...
import { Search, Star, BookOpen, Play, ChevronRight, Plus, Edit, Trash2, Eye, Folder, Loader2, DollarSign, Users, Dumbbell, Sparkles } from "lucide-react";
import { SkeletonGrid } from "@/components/ui/Skeleton";
import { cn, formatPrice, getSpecializationLabel, getCategoryColor, getInitials } from "@/lib/utils";
import { useAuthStore } from "@/lib/store/auth-store";
import { toast } from "sonner";

type ViewMode = "catalog" | "platform" | "my" | "purchased";

export default function LessonsPage() {
  const [catalog, setCatalog] = useState<any[]>([]);
  const [platformLessons, setPlatformLessons] = useState<any[]>([]);
  ...
  // Ommaviy ro'yxatlar: ikkala so'rov BIR VAQTDA ketadi (avval ketma-ket edi: 3 × ~1.5 s)
  useEffect(() => {
    let alive = true;
    (async () => {
      try {
        const [a, b] = await Promise.all([fetch("/api/lessons"), fetch("/api/lessons?platform=true")]);
        const [data, dataP] = await Promise.all([a.json(), b.json()]);
        if (!alive) return;
        setCatalog(Array.isArray(data) ? data : []);
        setPlatformLessons(Array.isArray(dataP) ? dataP : []);
      } catch (e) {} finally { if (alive) setLoading(false); }
    })();
    return () => { alive = false; };
  }, []);

  // O'z darsliklarim: foydalanuvchi ma'lumoti yuklangach so'raladi (sahifa yangilanganda ham ishlaydi)
  // va katalogni kutdirmaydi.
  useEffect(() => {
    if (!isTrainer || !user) return;
    let alive = true;
    fetch(`/api/lessons?trainer_id=${user.id}`)
      .then((r) => r.json())
      .then((d) => { if (alive) setMyLessons(Array.isArray(d) ? d : []); })
      .catch(() => {});
    return () => { alive = false; };
  }, [isTrainer, user?.id]);
    ...
    <div className="container-main py-8">
      ...
      {user && (
        <div className="flex gap-2 mb-6 overflow-x-auto">
          ...
          <TBtn active={viewMode === "platform"} onClick={() => setViewMode("platform")} icon={Sparkles} label="Platforma darsliklari" />
      ...
      {viewMode === "platform" && (
        <>
          <div className="bg-gradient-to-r from-lime/10 to-transparent border border-lime/20 rounded-xl p-4 mb-4 flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-lime/15 flex items-center justify-center shrink-0">
              <Sparkles className="h-5 w-5 text-lime" />
            </div>
            <div>
              <h3 className="text-sm font-semibold text-lime">Platforma premium darsliklari</h3>
              <p className="text-xs text-white/40">TrainerTop jamoasi tomonidan tayyorlangan maxsus darsliklar</p>
            </div>
          </div>
          {platformLessons.length === 0 ? (
            <div className="card p-12 text-center"><Sparkles className="h-10 w-10 text-white/10 mx-auto mb-4" /><p className="text-sm text-white/40">Hali platforma darsligi yo'q</p></div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{platformLessons.map((l) => <LessonCard key={l.id} lesson={l} />)}</div>
          )}
        </>
      )}
    ...
    <Link href={`/lessons/${l.id}`} className="card-hover group overflow-hidden">
      ...
      <div className="p-4">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            {l.total_sales > 0 && <span className="flex items-center gap-1 text-[11px] text-white/40"><Users className="h-3 w-3" />{l.total_sales}</span>}
