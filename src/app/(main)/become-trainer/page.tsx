...
export default function BecomeTrainerPage() {
  ...
  const handleBecomeTrainer = async () => {
    if (!user) {
      router.push("/login?redirect=/become-trainer");
      return;
    }

    setIsLoading(true);
    try {
      // Admin trener bo'lolmaydi (adminlik huquqi saqlanadi) — server ham buni tekshiradi
      if (user.role === "admin") {
        toast.error("Admin foydalanuvchi trener bo'la olmaydi");
        return;
      }

      // Rol o'zgarishi SERVER orqali (brauzerdan to'g'ridan-to'g'ri yozilmaydi)
      const res = await fetch("/api/become-trainer", { method: "POST" });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.message || "Trener bo'lishda xatolik");
      if (data.role === "admin") {
        toast.error("Admin foydalanuvchi trener bo'la olmaydi");
        return;
      }
      ...
      const tpRes = await fetch("/api/trainer-profile");
      if (tpRes.ok) setTrainerProfile((await tpRes.json()) as any);
