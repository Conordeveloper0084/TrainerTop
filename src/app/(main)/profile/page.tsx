...
import { fetchMyProfile, fetchMyTrainerProfile } from "@/lib/my-profile";
...
export default function ProfilePage() {
  ...
  return (
    <div className="container-main py-8">
      <div className="card p-6 mb-6">
        <div className="flex items-start gap-5">
          ...
          <div className="flex-1 pt-1">
            <h1 className="text-lg font-bold flex items-center gap-2">{user.full_name}{isTrainer && trainerProfile?.is_verified && <AthleteBadge size={20} />}{!isTrainer && (user as any).athlete_badge && <AthleteBadge size={20} kind="athlete" />}</h1>
            {user.username && <p className="text-xs text-lime/70">@{user.username}</p>}
      ...
      <Link href="/profile/followers" data-testid="followers-link" className="card p-4 mb-6 flex items-center gap-3 hover:border-lime/30 transition-colors">
        <Users className="h-4 w-4 text-lime" /><span className="text-sm">Obunachilarim</span>
        <span className="ml-auto text-sm font-bold">{isTrainer ? trainerProfile?.followers_count || 0 : (user as any).followers_count || 0}</span><ChevronRight className="h-4 w-4 text-white/30" />
      </Link>
...
function SettingsTab({ user, trainerProfile, isTrainer, onLogout }: any) {
  ...
  const [students, setStudents] = useState((trainerProfile?.manual_students ?? 0).toString());
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isTrainer) body.trainer_data = {
        ...
        manual_students: students ? Math.max(0, parseInt(students) || 0) : 0,
      };
      const res = await fetch("/api/profile", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      if (!res.ok) throw new Error("Xatolik");
      toast.success("Saqlandi!");
      const supabase = createClient();
      const p = await fetchMyProfile(supabase, user.id);
      if (p) useAuthStore.getState().setUser(p);
      if (isTrainer) { const tp = await fetchMyTrainerProfile(supabase, user.id); if (tp) useAuthStore.getState().setTrainerProfile(tp); }
    ...
    <div className="space-y-4">
      ...
      {isTrainer && (<>
        <div className="card p-6"><h3 className="font-semibold text-sm mb-4 flex items-center gap-2"><Dumbbell className="h-4 w-4 text-lime" />Trener</h3><div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4"><div><label className="block text-xs text-white/40 mb-2">Yosh</label><input type="number" value={age} onChange={(e) => setAge(e.target.value)} className="input-field" /></div><div><label className="block text-xs text-white/40 mb-2">Tajriba (yil)</label><input type="number" value={exp} onChange={(e) => setExp(e.target.value)} className="input-field" /></div><div><label className="block text-xs text-white/40 mb-2">Oldingi shogirdlar</label><input type="number" min="0" value={students} onChange={(e) => setStudents(e.target.value)} className="input-field" placeholder="0" /></div></div>
          <p className="text-[10px] text-white/30 -mt-2">"Oldingi shogirdlar" — platformadan tashqarida o'qitgan shogirdlaringiz soni. Platformada darslik sotib olganlar unga avtomatik qo'shiladi{typeof trainerProfile?.total_students === "number" ? ` (hozir jami: ${trainerProfile.total_students})` : ""}.</p>
...
// Settings ichidagi username (@handle) — ixtiyoriy, o'z holicha saqlanadi
function UsernameField({ user }: { user: any }) {
  const [value, setValue] = useState(user.username || "");
  const [saving, setSaving] = useState(false);
  const saved = user.username || "";
  const trimmed = value.trim();
  const validFormat = trimmed === "" || /^[A-Za-z0-9_]{3,20}$/.test(trimmed);
  const dirty = trimmed !== saved;

  const save = async () => {
    if (!dirty || !validFormat || saving) return;
    setSaving(true);
    try {
      const res = await fetch("/api/profile/username", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username: trimmed || null }) });
      const d = await res.json().catch(() => ({}));
      if (!res.ok) { toast.error(d.message || "Saqlanmadi"); return; }
      useAuthStore.getState().setUser({ ...user, username: d.username ?? null });
      setValue(d.username || "");
      toast.success(d.username ? "Username saqlandi" : "Username o'chirildi");
    } catch { toast.error("Saqlanmadi"); } finally { setSaving(false); }
  };

  return (
    <div className="card p-6">
      <h3 className="font-semibold text-sm mb-1">Username</h3>
      <p className="text-xs text-white/40 mb-3">Ixtiyoriy. Profilingiz havolasi va qidiruvda ishlatiladi (3-20 belgi: lotin harf, raqam, _).</p>
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-white/30 text-sm">@</span>
          <input type="text" value={value} onChange={(e) => setValue(e.target.value.replace(/\s/g, ""))} placeholder="username" maxLength={20} aria-label="Username" className="input-field !pl-8" />
        </div>
        <button onClick={save} disabled={!dirty || !validFormat || saving} className="btn-lime !py-2.5 !px-5 text-xs shrink-0 flex items-center gap-1.5 disabled:opacity-40">
          {saving && <Loader2 className="h-3.5 w-3.5 animate-spin" />}Saqlash
        </button>
      </div>
      {!validFormat && <p className="text-[11px] text-red-400 mt-1.5">3-20 belgi, faqat lotin harf/raqam/pastki chiziq (_)</p>}
    </div>
  );
}
