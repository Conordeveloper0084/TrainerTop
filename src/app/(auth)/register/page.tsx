...
import { Eye, EyeOff, Dumbbell, User, Loader2, Mail } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const [sentTo, setSentTo] = useState<string | null>(null); // tasdiqlash xati yuborilgan email
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    try {
      ...
      // Email tasdiqlash YOQILGAN bo'lsa sessiya bo'lmaydi — "emailni tekshiring" ekrani ko'rsatiladi
      if (!authData.session) {
    ...
    return (
      <div className="text-center">
        <div className="w-16 h-16 rounded-2xl bg-lime-muted flex items-center justify-center mx-auto mb-5">
          <Mail className="h-7 w-7 text-lime" />
        </div>
        <h1 className="text-2xl font-bold mb-2">Emailingizni tekshiring</h1>
        <p className="text-white/50 text-sm mb-1">Tasdiqlash havolasini quyidagi manzilga yubordik:</p>
        <p className="font-medium text-sm mb-6 break-all">{sentTo}</p>
        <p className="text-white/40 text-xs mb-6">Havolani bosgach akkauntingiz faollashadi. Xat kelmasa, «Spam» papkasini ham tekshiring.</p>
        <ResendEmail email={sentTo} initialCooldown={60} />
        <div className="mt-6 text-sm text-white/40 space-y-2">
          <p><button type="button" onClick={() => setSentTo(null)} className="text-lime hover:underline">Email noto'g'ri kiritilganmi? Qayta kiriting</button></p>
          <p>Tasdiqlab bo'lgan bo'lsangiz — <Link href="/login" className="text-lime hover:underline">Kirish</Link></p>
        </div>
      </div>
    );
  }
