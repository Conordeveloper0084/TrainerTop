...
import { Bell, LogOut, User, Settings, ChevronDown, Dumbbell, Headphones, Sun, Moon, Menu, X } from "lucide-react";
import { toast } from "sonner";
import { cn, getInitials } from "@/lib/utils";
import { useAuthStore } from "@/lib/store/auth-store";
import { createClient } from "@/lib/supabase/client";
import { useChatUnread, useNotifications, notificationsPoller } from "@/lib/shared-poll";
...
export default function Navbar() {
  ...
  // Chat unread — barcha komponentlar uchun bitta umumiy so'rov (30 soniyada bir)
  const chatUnread = useChatUnread(loggedIn);
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/[0.06] bg-dark/80 backdrop-blur-xl">
      <div className="container-main">
        <div className="flex h-16 items-center justify-between">
          ...
          {/* Planshet: support ikonkalari yonida + burger (< lg) */}
          <div className="flex lg:hidden items-center gap-1">
            {/* Planshetda ko'rinadi (md-lg), telefonda burger ichida */}
            ...
            {/* Burger tugmasi — planshet va telefonda */}
            <button onClick={() => setMobileOpen(!mobileOpen)}
              className="p-2 rounded-lg text-white/70 hover:text-white hover:bg-white/5 transition-colors ml-1"
              aria-label="Menu">
        ...
        {/* Burger ochiladigan panel */}
        {mobileOpen && (
          <div className="lg:hidden border-t border-white/[0.06] py-3 animate-fade-in">
            {/* Navigatsiya havolalari */}
            <div className="flex flex-col gap-1 mb-3">
              {links.map((l) => (
                <Link key={l.href + l.label} href={l.href} onClick={() => setMobileOpen(false)}
                  className={cn("px-4 py-2.5 rounded-lg text-sm transition-colors flex items-center justify-between",
                    active(l.href) ? "text-lime bg-lime-muted" : "text-white/60 hover:text-white hover:bg-white/5")}>
                  {l.label}
                  {l.href === "/chat" && chatUnread > 0 && (
                    <span className="min-w-[18px] h-[18px] rounded-full bg-red-500 text-white text-[9px] font-bold flex items-center justify-center px-1">{chatUnread}</span>
                  )}
                </Link>
              ))}
            </div>

            {/* Telefonda: notification, theme, support (planshetda yuqorida ko'ringani uchun bu yerda faqat telefonda) */}
            <div className="flex md:hidden items-center gap-2 px-4 py-2 border-t border-white/[0.06]">
              {loggedIn && <NotifBell loggedIn={loggedIn} />}
              <ThemeToggle />
              <Link href="/support" onClick={() => setMobileOpen(false)} className="p-2 rounded-lg text-white/30 hover:text-lime hover:bg-lime-subtle transition-colors" title="Yordam">
                <Headphones className="h-4 w-4" />
              </Link>
              <span className="text-xs text-white/40">Bildirishnoma, mavzu, yordam</span>
            </div>
            ...
            {ready && (loggedIn ? (
              <div className="border-t border-white/[0.06] pt-2 mt-1">
                ...
                <Link href="/profile" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-white/60 hover:text-white hover:bg-white/[0.04]">
                  <User className="h-4 w-4" />Profil
                </Link>
                {!isTrainer && (
                  <Link href="/become-trainer" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-lime/70 hover:text-lime hover:bg-lime-subtle">
                    <Dumbbell className="h-4 w-4" />Trener bo'lish
                  </Link>
                )}
                <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-2.5 text-sm text-red-400/70 hover:text-red-400 hover:bg-red-400/[0.04] w-full">
                  <LogOut className="h-4 w-4" />Chiqish
                </button>
              </div>
            ) : (
              <div className="border-t border-white/[0.06] pt-3 mt-1 flex gap-2 px-4">
                <Link href="/login" onClick={() => setMobileOpen(false)} className="flex-1 text-center px-4 py-2 text-sm text-white/60 hover:text-white border border-white/[0.08] rounded-lg">Kirish</Link>
                <Link href="/register" onClick={() => setMobileOpen(false)} className="flex-1 text-center btn-lime !py-2 text-sm">Boshlash</Link>
              </div>
            ))}
          </div>
        )}
...
function NotifBell({ loggedIn }: { loggedIn: boolean }) {
  ...
  // Bildirishnomalar hamma nusxalar uchun bitta umumiy so'rov bilan yangilanadi (60 soniyada bir)
  const notifs = useNotifications(loggedIn);
  const count = notifs.filter((n: any) => !n.is_read).length;
  const realUnread = notifs.filter((n: any) => !n.is_read && !n.sticky).length;   // yopishqoq (rasmiy kanal) qatorlar faqat kanal ochilganda o'qiladi
  ...
  const markRead = async () => {
    ...
    notificationsPoller.set(notifs.map((n: any) => ({ ...n, is_read: n.sticky ? n.is_read : true })));
  };
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => { setOpen(!open); if (!open && realUnread > 0) markRead(); }}
        aria-label="Bildirishnomalar" className="p-2 rounded-lg text-white/30 hover:text-lime hover:bg-lime-subtle transition-colors relative">
        ...
        <div className="absolute right-0 top-full mt-2 w-72 bg-dark-surface border border-white/[0.08] rounded-xl shadow-elevated overflow-hidden animate-fade-in z-50">
          ...
          <div className="max-h-64 overflow-y-auto">
            {notifs.length === 0 ? <p className="text-xs text-white/20 text-center py-6">Hali bildirishnoma yo'q</p> : notifs.slice(0, 10).map((n) => (
              n.href && typeof n.href === "string" && n.href.startsWith("/") && !n.href.startsWith("//") ? (
                <Link key={n.id} href={n.href} onClick={() => setOpen(false)} className={cn("block px-4 py-3 border-b border-white/[0.03] text-xs hover:bg-white/[0.03]", !n.is_read && "bg-lime/[0.03]")}>
